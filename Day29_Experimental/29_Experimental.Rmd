...
```{r data-prep}
df_popgrid <-
  readr::read_csv(here::here("_dev", "gpwv4-2015-merc.csv"))

df_popgrid_clean <-
  df_popgrid %>% 
  filter(!(x < 260 & y < 260), y < 1500)
```
...
x25 <-
  ...
  ggplot(aes(x_factor, -y)) + 
    ...
    scale_color_manual(values = c("#ebeae1", "#f8d9af", "#ffb793", "#f56965", "#d92864"),
                       name = "Population distribution per 25 km latitudinal stripe",
    ...
    labs(title = "World Population Stripes",
         subtitle = "This map shows the world population along a latitudinal gradient from North to South. Each stripe represents the distribution of the human population and covers 25 km in width.",
...
x10 <-
  ...
  ggplot(aes(x_factor, -y)) + 
    ...
    scale_color_manual(values = c("#edecdf", "#f8d9af", "#ffb793", "#f56965", "#d92864"),
                       name = "Population distribution per 10 km latitudinal stripe",
                       labels = c("100%", "75%", "50%", "25%", "10%")) +
    guides(color = guide_legend(title.position = "left", 
                                title.hjust = 0.5, ncol = 1,
                                label.position = "right",
...
x50 <-
  ...
  ggplot(aes(x_factor, -y)) + 
    ...
    scale_color_manual(values = c("#f1efdb", "#f8d9af", "#ffb793", "#f56965", "#d92864"),
                       name = "Population distribution per 50 km latitudinal stripe",
                       labels = c("100%", "75%", "50%", "25%", "10%")) +
    guides(color = guide_legend(title.position = "left", 
                                title.hjust = 0.5, ncol = 1,
                                label.position = "right",
