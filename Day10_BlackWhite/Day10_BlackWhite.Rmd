...
## map with dark grey
ggplot(sf_world) + 
  geom_sf(fill = "black", color = "grey20", size = 0.01) +
  ...
  labs(title = "Urban Population of the World's Capital Cities",
       caption = "Visualization by Cédric Scherer  •  Data by Natural Earth Data & SimpleMaps.com") +
...
## map bw only
ggplot(sf_world) + 
  geom_sf(fill = "black", color = "white", size = 0.01) +
  geom_sf(data = goode_without, fill = "white", color = NA) +
  geom_sf(data = goode_outline, fill = NA, color = "black", size = 0.1) +
  geom_sf(data = sf_capitals, 
          aes(size = Population),
