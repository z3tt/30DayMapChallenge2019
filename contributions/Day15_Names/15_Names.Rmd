...
sf_bln_roads_classified %>% 
  ggplot() + 
    ...
    annotate("text", x = 13.45, y = 52.344, 
             ...
             #label = "Visualisierung: Cédric Scherer  •  Daten: OpenStreetMap",
             family = "Oswald", 
             size = 4.5, 
             color = "grey60") + 
    labs(title = "NAMES OF ROADS IN BERLIN",
         caption = '¹ Road type names that are related to close-by water bodies, i.e. **"See" (lake)**, **"Ufer" (shore)**, and **"Steg" (footbridge)**<br>² Other road type names include, e.g. **"Promenade" (esplanade)**, **"Pfad" (trail)**, **"Ring" (ring road)**, **"Brücke" (bridge)**, **"Graben" (trench)**, and **"Hof" (courtyard)**') +
    theme(plot.caption = element_markdown(margin = margin(0, 12, 6, 120)))
    # labs(title = "BERLINER STRAßENNAMEN",
    #      caption = '¹ Straßennamen, die mit Gewässern in Verbindung stehen, z.B. **"See"**, **"Ufer"** und **"Steg"**<br>² Andere Namen für Straßen, z.B **"Promenade"**, **"Pfad"**, **"Ring"**, **"Zeile"**, **"Gasse"**, **"Brücke"**, **"Graben"** und **"Hof"**') +
    # theme(plot.caption = element_markdown(margin = margin(0, 12, 6, 220)))

ggsave(here::here("contributions", "Day15_Names", "Names_BerlinRoads_en.pdf"),
       width = 12, height = 11.67, device = cairo_pdf)
# ggsave(here::here("contributions", "Day15_Names", "Names_BerlinRoads_de.pdf"),
#        width = 12, height = 11.615, device = cairo_pdf)
```
