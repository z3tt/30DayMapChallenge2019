---
title: "#30DayMapChallenge Day 3: Polygons - Gobal Mortality by OurWorldInData.org"
...
ggsave(here::here("contributions", "Day03_Polygons", "Polygons_GlobalMortality.pdf"), 
       width = 18, height = 12.5, device = cairo_pdf)
```

```{r choropleths}
map_cdv_c <- 
  base_map +
  geom_sf(data = sf_mortality, 
          aes(geometry = geometry, 
              fill = cardiovascular_diseases_percent),
          color = "transparent", size = 0.1) +
  labs(subtitle = "Cardiovascular Diseases")
...
```{r full-panel-choropleth, fig.width = 18, fig.height = 12.5}
map_cdv_c + map_cancer_c + 
  (map_diabetes_c + theme(plot.background = element_rect(fill = NA, color = NA), panel.background = element_rect(fill = NA, color = NA))) + 
  map_hiv_c + map_malaria_c + map_conflict_c + 
  plot_layout(ncol = 3)

ggsave(here::here("contributions", "Day03_Polygons", "Polygons_GlobalMortality_Choropleth.pdf"), 
       width = 18, height = 12.5, device = cairo_pdf)
```
...
ggsave(here::here("3_Polygons", "Polygons_Alcohol_Drugs.pdf"), 
       width = 11, height = 14, device = cairo_pdf)
```
